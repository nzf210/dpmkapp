...
import DocSp2d_add from './DocSp2d';
...
const Sp2dAdd = () => {
    ...
    useEffect(async () => {
        setLoad(true);
        let url = `/anggaran?page=${page}&size=${perpage}&sts_spp=true&sts=true&sts_spm=true&kd_keg=4&sts_sp2d=true`;
    ...
    useEffect(async () => {
        setLoad(true);
        let url = `/anggaran?page=${page_}&size=${perpage_}&sts_spp=true&sts=true&sts_spm=true&kd_keg=4&sts_sp2d=false`;
        if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
    ...
    const updateDataChecklist = async () => {
        ...
        setLoad(true);
        const nomor = await axios.get(`/nodok?kd_keg=4`);
    ...
    const numbering = (v) => {
        switch (true) {
            case v < 10:
                return `000${v}/SP2D-ALOKASIDANADESA/2022`;
            case v < 100:
                return `00${v}/SP2D-ALOKASIDANADESA/2022`;
            case v < 1000:
                return `0${v}/SP2D-ALOKASIDANADESA/2022`;
            default:
                return `${v}/SP2D-ALOKASIDANADESA/2022`;
            //break;
        }
    }
    ...
    const btnClick = async (e) => {
        setLoad(true);
        let url = `/anggaran?page=${1}&size=${perpage}&kampung=${search}&sts_spp=${true}&sts=${true}&sts_spm=true&kd_keg=4&sts_sp2d=true`;
        if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
    ...
    const btnClick_ = async (e) => {
        setLoad(true);
        let url = `/anggaran?page=${1}&size=${perpage_}&kampung=${search_}&sts_spp=${true}&sts=${true}&sts_spm=true&kd_keg=4&sts_sp2d=false`;
        if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
        // if (search) { url += `&kd_advis=${}` }   
        await axios.get(url).then((e) => {
            setRowData_(e.data.result.data.data);
                ...
                <div className='fixed  mt-10 w-full z-50 flex'>
                    ...
                    <div className='grow'>
                        <div className='mx-auto justify-center items-center h-screen w-[90%]'>
                            ...
                            ><DocSp2d_add dataselectspp={dataVprint} /></PDFViewer>
            ...
            <div className='grid grid-flow-row h-0'>
                ...
                <div >
                    ...
                    <div className="flex items-center justify-center z-10">
                        ...
                        {print ? <div>
                            {viewbtn ?
                                <><PDFDownloadLink placeholder='Print Data PDF' document={<DocSp2d_add dataselectspp={dataDlprint} />} fileName={`doc_spp-reg_${new Date().toLocaleTimeString().slice(0, 16)}`}>
...
//                         break;
//                     case (9 < nodok < 100):
//                         nomor = `00${nodok}/SP2D-ALOKASIDANADESA/2022`;
//                         console.log('>9', nomor);
//                         break;
//                     case (99 > nodok > 1000):
//                         nomor = `0${nodok}/SP2D-ALOKASIDANADESA/2022`;
//                         console.log('>99');
//                         break;
//                     case (999 > nodok > 9999):
//                         nomor = `${nodok}/SP2D-ALOKASIDANADESA/2022`;
//                         console.log('>999');
//                         break;
//                     default:
//                         break;
...
//                     id: f.id, tgl_sp2d: ee.toISOString().slice(0, 10), sts_sp2d: eee, no_sp2d: nomor
//                 })
...
//     const action = [
//         {
//             icon: () => <><PDFDownloadLink document={<DocSp2d dataselectspp={dataselectspp} />} fileName={`doc_spd_${new Date().toLocaleTimeString().slice(0, 16)}`}><LocalPrintshopIcon /></PDFDownloadLink></>,
//             tooltip: 'Download Dok SP2D',
//             onClick: ''
//         },
//         {
//             icon: () => <div className='flex'><button onClick={() => previewSpd()} className="mr-2" > <PictureAsPdfIcon /></button></div>,
//             tooltip: 'Preview SP2D',
//             onClick: ''
//         },
//     ]
...
//                                 ><DocSp2d dataselectspp={dataselectspp} /></PDFViewer> : null}
//                         </div>
//                     </div>
//                 </div>
//             </div>
...
//                                 />
//                                 <br />
//                                 {kd_lvl1 !== 2 ? <p className='text-blue-700'>*Note: Silahkan Pilih Kegiatan untuk terbitkan SP2D </p> : null}
//                                 <MaterialTable
//                                     options={options_}
//                                     icons={tblIcon}
//                                     columns={kolom_}
