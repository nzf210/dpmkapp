...
import DocSp2dReg from './DocSp2d_reg';
...
const Sp2dReguler = () => {
    ...
    // useEffect(async () => {
    //     setLoad(true);
    //     let url = `/anggaran/reg?page=${page_}&size=${perpage_}&sts_spp=true&sts=true&sts_spm=true&kd_keg=1&sts_sp2d=false`;
    //     if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
    ...
    const numbering = (v, vv) => {
        ...
        switch (true) {
            case v < 10:
                return `000${v}/SP2D-DDSREGULER ${no}/2022`;
            case v < 100:
                return `00${v}/SP2D-DDSREGULER ${no}/2022`;
            case v < 1000:
                return `0${v}/SP2D-DDSREGULER ${no}/2022`;
            default:
                return `${v}/SP2D-DDSREGULER ${no}/2022`;
            //break;
        }
    }
    ...
    const btnClick = async () => {
        setLoad(true);
        let url = `/anggaran/reg?page=${1}&size=${perpage}&kampung=${search}&sts_spp=${true}&sts=${true}&sts_spm=true&kd_keg=1&sts_sp2d=true`;
        if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
    ...
    const btnClick_ = async () => {
        setLoad(true);
        let url = `/anggaran/reg?page=${1}&size=${perpage_}&kampung=${search_}&sts_spp=${true}&sts=${true}&sts_spm=true&kd_keg=1&sts_sp2d=false`;
        if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
        // if (search) { url += `&kd_advis=${}` }   
        await axios.get(url).then((e) => {
            setRowData_(e.data.result.data.data);
                ...
                <div className='fixed  mt-10 w-full z-50 flex'>
                    ...
                    <div className='grow'>
                        <div className='mx-auto justify-center items-center h-screen w-[90%] relative'>
                            ...
                            ><DocSp2dReg dataselectspp={dataVprint} nama={nama} dataKadis={dataKadis} /></PDFViewer>
            ...
            <div className='grid grid-flow-row h-0'>
                ...
                <div >
                    ...
                    <div className="flex items-center justify-center z-10">
                        ...
                        {print ? <div>
                            {viewbtn ?
                                <><PDFDownloadLink placeholder='Print Data PDF' document={<DocSp2dReg dataselectspp={dataDlprint} nama={nama} dataKadis={dataKadis} />} fileName={`doc_spp-reg_${new Date().toLocaleTimeString().slice(0, 16)}`}>
