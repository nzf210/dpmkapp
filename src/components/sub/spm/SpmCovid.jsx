...
const SpmCovid = () => {
    ...
    const btnClick = async (e) => {
        setLoad(true);
        let url = `/anggaran/cov?page=${1}&size=${perpage}&kampung=${search}&sts_spp=${true}&sts=${true}&sts_spm=true`;
        if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
    ...
    const btnClick_ = async (e) => {
        setLoad(true);
        let url = `/anggaran/cov?page=${1}&size=${perpage_}&kampung=${search_}&sts_spp=${false}&sts=${true}&sts_spm=true`;
        if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
        // if (search) { url += `&kd_advis=${}` }   
        await axios.get(url).then((e) => {
            setRowData_(e.data.result.data.data);
