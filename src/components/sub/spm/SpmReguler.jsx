...
import DocSpmreg from './DocSpm_reg';
...
const SpmRegule = () => {
    ...
    // useEffect(async () => {
    //     setLoad(true);
    //     let url = `/anggaran/reg?page=${page_}&size=${perpage_}&sts_spp=true&sts=true&sts_spm=false`;
    //     if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
            ...
            if (confirm) {
                ...
                try {
                    for (let i = 0; i < len; i++) {
                        ...
                        if (!sts_sp2d) {
                            setLoad(true);
                            await axios.patch('/anggaran', { id, tgl_spm, sts_spm: false, no_spm: null })
                            if ((i + 1) === len) {
                                setInfo('Data Di Hapus');
                                setDialogInfo(true);
                                setTimeout(() => {
    ...
    const btnClick = async (e) => {
        setLoad(true);
        let url = `/anggaran/reg?page=${1}&size=${perpage}&kampung=${search}&sts_spp=${true}&sts=${true}&sts_spm=true`;
        if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
    ...
    const btnClick_ = async (e) => {
        setLoad(true);
        let url = `/anggaran/reg?page=${1}&size=${perpage_}&kampung=${search_}&sts_spp=${false}&sts=${true}&sts_spm=true`;
        if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
        // if (search) { url += `&kd_advis=${}` }   
        await axios.get(url).then((e) => {
            setRowData_(e.data.result.data.data);
                ...
                <div className='fixed  mt-10 w-full z-50 flex'>
                    ...
                    <div className='grow'>
                        <div className='mx-auto justify-center items-center h-screen w-[90%] relative'>
                            ...
                            ><DocSpmreg dataselectspp={dataVprint} /></PDFViewer>
            ...
            <div className='grid grid-flow-row h-0'>
                ...
                <div >
                    ...
                    <div className="flex items-center justify-center z-10">
                        ...
                        {print ? <div>
                            {viewbtn ?
                                <><PDFDownloadLink placeholder='Print Data PDF' document={<DocSpmreg dataselectspp={dataDlprint} />} fileName={`doc_spp-reg_${new Date().toLocaleTimeString().slice(0, 16)}`}>
