...
const SppAdd = () => {
    ...
    useEffect(async () => {
        setLoad(true);
        let url = `/anggaran?page=${page}&size=${perpage}&sts_spp=${true}&sts=${true}&kd_keg=4`;
    ...
    useEffect(async () => {
        setLoad(true);
        let url = `/anggaran?page=${page_}&size=${perpage_}&sts_spp=${false}&sts=${true}&kd_keg=4`;
        if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
    ...
    const updateDataChecklist = async () => {
        ...
        try {
            dataprint_.map(async (f, i) => {
                ...
                switch (true) {
                    case (nodok < 10):
                        nomor = `000${nodok}/SP2SPD/ADD-${thp}/${f.kampung}/2022`; //`000${nodok}/SP2SPD/ADD1/${f.kampung}/2022`;
                        console.log('<9', nomor);
                        break;
                    case (9 < nodok < 100):
                        nomor = `00${nodok}/SP2SPD/ADD-${thp}/${f.kampung}/2022`;
                        console.log('>9', nomor);
                        break;
                    case (99 > nodok > 1000):
                        nomor = `0${nodok}/SP2SPD/ADD-${thp}/${f.kampung}/2022`;
                        console.log('>99');
                        break;
                    case (999 > nodok > 9999):
                        nomor = `${nodok}/SP2SPD/ADD-${thp}/${f.kampung}/2022`;
                        console.log('>999');
                        break;
                    default:
                        break;
    ...
    const btnClick = async (e) => {
        setLoad(true);
        let url = `/anggaran?page=${1}&size=${perpage}&kampung=${search}&sts_spp=${true}&sts=${true}&kd_keg=4`;
        if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
    ...
    const btnClick_ = async (e) => {
        setLoad(true);
        let url = `/anggaran?page=${1}&size=${perpage_}&kampung=${search_}&sts_spp=${false}&sts=${true}&kd_keg=4`;
        if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
        // if (search) { url += `&kd_advis=${}` }   
        await axios.get(url).then((e) => {
            setRowData_(e.data.result.data.data);
        ...
        <>
                ...
                <div className='fixed  mt-10 w-full z-50 flex'>
                    ...
                    <div className='grow'>
                        <div className='mx-auto justify-center items-center h-screen w-[90%]'>
                            ...
                            ><DocSpp_add dataselectspp={dataVprint} /></PDFViewer>
            ...
            <div className='grid grid-flow-row h-0'>
                ...
                <div >
                    ...
                    <span className='left-0 w-44 absolute ml-[20%] -z-50 font-semibold text-slate-900'>SP2SPD ADD TERBIT</span>
                    <div className="flex items-center justify-center z-10">
                        ...
                        {print ? <div>
                            {viewbtn ?
                                <><PDFDownloadLink placeholder='Print Data PDF' document={<DocSpp_add dataselectspp={dataDlprint} />} fileName={`doc_spp-reg_${new Date().toLocaleTimeString().slice(0, 16)}`}>
                ...
                <div className='mt-72 -z-20'>
                    <span className='left-0 w-44 absolute ml-[20%] -z-50 font-semibold text-slate-900'>SP2SPD ADD PROSES</span>
                    <div className="flex items-center justify-center z-10">
                        <div className="flex border-2 rounded">
