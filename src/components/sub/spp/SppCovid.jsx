...
import DocSppcov from './DocSpp_cov';
...
const SppCovid = () => {
    ...
    // useEffect(async () => {
    //     setLoad(true);
    //     let url = `/anggaran/cov?page=${page}&size=${perpage}&sts_spp=${true}&sts=${true}`;
    ...
    // useEffect(async () => {
    //     setLoad(true);
    //     let url = `/anggaran/cov?page=${page_}&size=${perpage_}&sts_spp=${false}&sts=${true}`;
    //     if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
            ...
            if (confirm) {
                ...
                try {
                    ...
                    for (let i = 0; i < len; i++) {
                        ...
                        if (!sts_spm) {
                            setLoad(true);
                            await axios.patch('/anggaran', { id, tgl_spp: null, sts_spp: false, no_spp: null })
                            if ((i + 1) === len) {
                                setInfo('Data Di Hapus');
                                setDialogInfo(true);
                                setTimeout(() => {
    ...
    const btnClick = async (e) => {
        setLoad(true);
        let url = `/anggaran/cov?page=${1}&size=${perpage}&kampung=${search}&sts_spp=${true}&sts=${true}`;
        if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
    ...
    const btnClick_ = async (e) => {
        setLoad(true);
        let url = `/anggaran/cov?page=${1}&size=${perpage_}&kampung=${search_}&sts_spp=${false}&sts=${true}`;
        if (kd_lvl1 === 2) { url += `&kd_kampung=${kd_kampung}` }
        // if (search) { url += `&kd_advis=${}` }   
        await axios.get(url).then((e) => {
            setRowData_(e.data.result.data.data);
        ...
        <>
                ...
                <div className='fixed  mt-10 w-full z-50 flex'>
                    ...
                    <div className='grow'>
                        <div className='mx-auto justify-center items-center h-screen w-[90%] relative'>
                            ...
                            ><DocSppcov dataselectspp={dataVprint} /></PDFViewer>
            ...
            <div className='grid grid-flow-row h-0'>
                ...
                <div >
                    ...
                    <span className='left-0 w-44 absolute ml-[20%] -z-50 font-semibold text-slate-900'>SP2SPD COVID TERBIT</span>
                    <div className="flex items-center justify-center z-10">
                        ...
                        {print ? <div>
                            {viewbtn ?
                                <><PDFDownloadLink placeholder='Print Data PDF' document={<DocSppcov dataselectspp={dataDlprint} />} fileName={`doc_spp-reg_${new Date().toLocaleTimeString().slice(0, 16)}`}>
                ...
                <div className='mt-72 -z-20'>
                    <span className='left-0 w-44 absolute ml-[20%] -z-50 font-semibold text-slate-900'>SP2SPD COVID PROSES</span>
                    <div className="flex items-center justify-center z-10">
                        <div className="flex border-2 rounded">
